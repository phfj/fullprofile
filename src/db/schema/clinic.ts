...
export const accomodationPhotosTable = sqliteTable('accomodationPhotos', {
    id: text('id').primaryKey(),
    accomodationId: text('id').notNull().references(() => accomodationTable.id, { onDelete: 'cascade' }),
    url: text('url').notNull(),
    alt: text('alt'),
    createdAt: text('createdAt').notNull().$default(() => new Date().toISOString()),
    updatedAt: text('udpatedAt').notNull().$default(() => new Date().toISOString()).$onUpdateFn(() => new Date().toISOString()),
});